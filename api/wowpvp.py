...
def update_realms():
    ...
    for region in regions:
        ...
        realms = make_request('https://' + region.name + '.api.blizzard.com/data/wow/realm/index', query)
...
def get_character(name, region, realm):
    ...
    if response:
        if 'code' in response and response['code'] == 404:
            filter = Character.objects.filter(name=name, realm=realm, region=region)
            if filter:
                char = Character.objects.get(name=name, realm=realm, region=region)
                char.delete()
            return False
...
def get_media(name, realm, region, cid):
    ...
    print('https://' + region.name + '.api.blizzard.com/profile/wow/character/' + realm.slug + '/' + name + '/character-media')
